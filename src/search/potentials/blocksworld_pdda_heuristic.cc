...
static pair<unique_ptr<PotentialFunctionFeatures>, unique_ptr<PotentialFunctionFeatures>> create_blocksworld_potential_function(const TaskProxy &task_proxy){
    VariablesProxy variables = task_proxy.get_variables();
    utils::HashMap<vector<pair<int, int>>, int> feature_potentials1;
    utils::HashMap<vector<pair<int, int>>, int> feature_potentials2;
    ...
    int num_blocks = all_blocks.size()+1;
    ...
    for(const auto &blocks1: on){
        ...
        if(lower_block1 != "table" && lower_block1 != upper_block1){
            feature_potentials2[{get<0>(blocks1)}] = -1;
        }

        if(lower_block1 == upper_block1){
            feature_potentials2[{get<0>(blocks1)}] = num_blocks+1;
        }

        if(lower_block1 != "table" && upper_block1 != lower_block1 && lower_block1 != goal_ons[upper_block1]){
            feature_potentials1[{get<0>(blocks1)}] = 2;
        }

        if(lower_block1 == "table" ){

            if(goal_ons[upper_block1] == "table"){
                feature_potentials1[{get<0>(blocks1)}] = 0;
            }
            else{
                feature_potentials1[{get<0>(blocks1)}] = 1;
            }
        }
        ...
        for(const auto &blocks2: on){
            ...
            if(upper_block1 != upper_block2 && lower_block1 != "table" && lower_block2 != "table" &&
                ...
                (goal_ons[upper_block1] != lower_block1 || goal_ons[upper_block2] != lower_block2)){
                    ...
                    feature_potentials2[{get<0>(blocks1),get<0>(blocks2)}] = num_blocks+1;
            }
            ...
            if(lower_block1 == goal_ons[upper_block1] && lower_block2 != goal_ons[upper_block2]){
                ...
                if (block_controls != controls.end()) {
                    ...
                    if (controlled) {
                        feature_potentials1[{get<0>(blocks1), get<0>(blocks2)}] = pow(2,goal_levels[upper_block1]);
                    }
                }
            }
        }
        ...
        for(const auto &block_clear: clear){
            ...
            if(is_clear && block == lower_block1){
                feature_potentials2[{get<0>(blocks1),get<0>(block_clear)}] = num_blocks;
            }
        }
    }
    ...
    for(const auto &block_clear: clear){
            ...
            if(!is_clear){
                feature_potentials2[{get<0>(block_clear)}] = 1;
            }
        }

    return std::make_pair(
    make_unique<PotentialFunctionFeatures>(move(feature_potentials1)),
    make_unique<PotentialFunctionFeatures>(move(feature_potentials2)));
}
class BlocksworldHeuristicPDDAFeature
    : public plugins::TypedFeature<Evaluator, PotentialHeuristicPDDA> {
public:
    BlocksworldHeuristicPDDAFeature() : TypedFeature("blocksworld_pdda_heuristic") {
        add_heuristic_options_to_feature(*this, "blocksworld pdda heuristic");
    }

    virtual shared_ptr<PotentialHeuristicPDDA>
    create_component(const plugins::Options &opts) const override {
        auto task_proxy = TaskProxy(*opts.get<shared_ptr<AbstractTask>>("transform"));
        auto functions = create_blocksworld_potential_function(task_proxy);
        auto function1 = std::move(functions.first);
        auto function2 = std::move(functions.second);
        return make_shared<PotentialHeuristicPDDA>(
            move(function1),
            move(function2),
...
static plugins::FeaturePlugin<BlocksworldHeuristicPDDAFeature> _plugin;
}